...
public static class TestSet___paramNullable
{
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => vv_src.AddMonths(1)==new System.DateTime(2018,11,9) && r.TEST_ID==testID);
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var sqlt
      =new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_INTEGER).T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (DATEADD(MONTH,").N("t",c_NameOf__COL_INTEGER).T(",").P_TS("__vv_src_0").T(") = timestamp '2018-12-09') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")");
 ...
 public static void Test___param___ByEF____2()
 {
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     T_DATA c_data=new System.DateTime(2019,1,9);
 ...
 //       .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_INTEGER).T(", ").N("t",c_NameOf__COL_DATA).EOL()
 //       .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
 //       .T("WHERE (DATEADD(MONTH,").P("__vv_1").T(" / ").N("t",c_NameOf__COL_INTEGER).T(",").P("__vv_src_0").T(") = timestamp '2018-11-09') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")")
 //       .BuildSql(cn));
 // 
 //    tr.Rollback();
 //   }//using tr
