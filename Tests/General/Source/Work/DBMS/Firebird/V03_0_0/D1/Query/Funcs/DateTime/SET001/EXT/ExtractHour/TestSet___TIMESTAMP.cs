 ...
 public static void Test___01___ByDBMS()
 {
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    {
     T_DATA c_data=new System.DateTime(2018,10,9,8,7,6);

     System.Int64? testID=Helper__InsertRow(db,c_data);

     var recs=db.testTable.Where(r => r.DATA.ExtractHour()==8 && r.TEST_ID==testID);
     ...
     var sqlt
      =new TestSqlTemplate()
        ...
        .T("WHERE (EXTRACT(HOUR FROM ").N("t",c_NameOf__COL_DATA).T(") = 8) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")");
