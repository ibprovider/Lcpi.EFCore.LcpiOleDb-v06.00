...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D1.Query.Operators.SET_002__AS_STR.Add.Complete.Int32.Int32{
////////////////////////////////////////////////////////////////////////////////
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (string)(object)(r.COL_DATA1+r.COL_DATA2)=="-4294967296.000000" && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (").PUSH().N("t",c_NameOf__COL_DATA1).T(" + ").N("t",c_NameOf__COL_DATA2).W_BRKTS().W_CastAsVCH(32).POP().T(" = _utf8 '-4294967296.000000') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (string)(object)(r.COL_DATA1+r.COL_DATA2)=="4294967294.000000" && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (").PUSH().N("t",c_NameOf__COL_DATA1).T(" + ").N("t",c_NameOf__COL_DATA2).W_BRKTS().W_CastAsVCH(32).POP().T(" = _utf8 '4294967294.000000') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
