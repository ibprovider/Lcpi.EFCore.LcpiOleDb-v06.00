...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D1.Query.Funcs.Querable.SET_001.STD.Sum{
////////////////////////////////////////////////////////////////////////////////
...
public static class TestSet_001__verify_result_type
{
 ...
 {
  new tagData("TEST_MODIFY_ROW"      ,"COL_SMALLINT",  xdb.OleDbType.Integer,   10,   null),
  new tagData("TEST_MODIFY_ROW"      ,"COL_INTEGER" ,  xdb.OleDbType.Integer,   10,   null),
  new tagData("TEST_MODIFY_ROW"      ,"COL_BIGINT"  ,  xdb.OleDbType.Double,    15,   null),
  ...
  new tagData("NUM"                  ,"N_1_0"       ,  xdb.OleDbType.Numeric,    9,   0),
  new tagData("NUM"                  ,"N_1_1"       ,  xdb.OleDbType.Numeric,    9,   1),

  new tagData("NUM"                  ,"N_9_0"       ,  xdb.OleDbType.Numeric,    9,   0),
  new tagData("NUM"                  ,"N_9_1"       ,  xdb.OleDbType.Numeric,    9,   1),

  new tagData("NUM"                  ,"N_18_17"     ,  xdb.OleDbType.Double,    15,   null),
  new tagData("NUM"                  ,"N_18_18"     ,  xdb.OleDbType.Double,    15,   null),
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    using(var cmd=cn.CreateCommand())
    {
     ...
     {
      string sql
       ="select SUM("+testData.ColumnName+") from "+testData.TableName;
      ...
      using(var reader=cmd.ExecuteReader(System.Data.CommandBehavior.SchemaOnly))
      {
       ...
       if(testData.ResultOleDbType != actualOleDbType)
       {
        string err
         =string.Format
          ("SUM({0}.{1}) -> type: {2}. Expected type: {3}.",
           testData.TableName,
           testData.ColumnName,
           actualOleDbType,
       ...
       if(testData.ResultPrecision != actualPrecision)
       {
        string err
         =string.Format
          ("SUM({0}.{1}) -> precision: {2}. Expected precision: {3}.",
           testData.TableName,
           testData.ColumnName,
           actualPrecision,
       ...
       if(testData.ResultScale != actualScale)
       {
        string err
         =string.Format
          ("SUM({0}.{1}) -> Scale: {2}. Expected Scale: {3}.",
           testData.TableName,
           testData.ColumnName,
           actualScale,
       ...
       {
        Console.WriteLine
         ("SUM({0}.{1}) -> OK. type: {2}, precision: {3}, scale: {4}",
          testData.TableName,
          testData.ColumnName,
          testData.ResultOleDbType,
          testData.ResultPrecision,
