  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => r.DATA.ExtractSecond()==15 && r.TEST_ID==testID);
     ...
     var sqlt
      =new TestSqlTemplate()
        ...
        .T("WHERE (CAST(TRUNC(EXTRACT(SECOND FROM ").N("t",c_NameOf__COL_DATA).T(")) AS INTEGER) = 15) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")");
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => xdbfunc.DbCast.AsInteger(r.DATA.ExtractSecond())==15 && r.TEST_ID==testID);
