    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (").PUSH().N("t",c_NameOf__COL1).T(" + ").N("t",c_NameOf__COL1).W_BRKTS().W_CastAsVCH(c_TARGET_VCH_LENGTH__DEC).POP().T(" = _utf8 '642.4') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL1).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (").PUSH().N("t",c_NameOf__COL1).T(" + ").N("t",c_NameOf__COL1).W_BRKTS().T(" + ").N("t",c_NameOf__COL1).W_BRKTS().W_CastAsDBL().W_CastAsVCH(c_TARGET_VCH_LENGTH__R8).POP().T(" = _utf8 '963.5999999999999') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL1).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (").PUSH().N("t",c_NameOf__COL1).T(" + ").N("t",c_NameOf__COL1).W_BRKTS().T(" + ").N("t",c_NameOf__COL1).W_BRKTS().W_CastAsDBL().T(" + ").N("t",c_NameOf__COL1).W_BRKTS().W_CastAsVCH(c_TARGET_VCH_LENGTH__R8).POP().T(" = _utf8 '1252.800000000000') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
