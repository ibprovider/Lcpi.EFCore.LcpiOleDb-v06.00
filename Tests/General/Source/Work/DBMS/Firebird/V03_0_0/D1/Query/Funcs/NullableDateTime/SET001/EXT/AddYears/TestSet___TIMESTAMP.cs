...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D1.Query.Funcs.NullableDateTime.SET001.EXT.AddYears{
////////////////////////////////////////////////////////////////////////////////
//using
...
public static class TestSet___TIMESTAMP
{
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => r.DATA.AddYears(null)==null && r.TEST_ID==testID);
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => r.DATA.AddYears(vv)==null && r.TEST_ID==testID);
 ...
 //       .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_INTEGER).T(", ").N("t",c_NameOf__COL_DATA).EOL()
 //       .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
 //       .T("WHERE (DATEADD(YEAR,(").P("__Exec_0").T(" / ").N("t",c_NameOf__COL_INTEGER).T("),").N("t",c_NameOf__COL_DATA).T(") = timestamp '2022-10-09') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")")
 //       .BuildSql(cn));
 // 
 //    tr.Rollback();
 //   }//using tr
