    ...
    TestServices.UnsupportedSQL__DataTypeUnknown
     (tr,
      new TestSqlTemplate()
       .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_INTEGER).T(", ").N("t",c_NameOf__COL_DATA).EOL()
       .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
       .T("WHERE (DATEADD(MILLISECOND,1000*CAST(").P("__Exec_0").T(" + ").N("t",c_NameOf__COL_INTEGER).T(" AS DOUBLE PRECISION),").N("t",c_NameOf__COL_DATA).T(") = timestamp '2018-10-09 00:00:07') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")")
       .BuildSql(cn));
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var sqlt
      =new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_INTEGER).T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (DATEADD(MILLISECOND,1000 * (CAST(").N("t",c_NameOf__COL_INTEGER).T(" AS DOUBLE PRECISION) / (CAST(2 AS DOUBLE PRECISION) * ").N("t",c_NameOf__COL_INTEGER).T(")),").N("t",c_NameOf__COL_DATA).T(") = timestamp '2018-10-09 00:00:00.5') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")");
