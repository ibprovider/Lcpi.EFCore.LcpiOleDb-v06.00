    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE ((").N("t",c_NameOf__COL_DATA1).T(" <> 3) OR (").N("t",c_NameOf__COL_DATA2).T(" <> 4)) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE ((").N("t",c_NameOf__COL_DATA1).T(" <> 22) OR (").N("t",c_NameOf__COL_DATA2).T(" <> 44)) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
    }//using db
