  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => r.DATA.AddDays((T_AMOUNT)r.AMOUNT)==new T_DATA_U(c_ticks) && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (DATEADD(MILLISECOND,86400000 * ").N("t",c_NameOf__COL_AMOUNT).T(",").N("t",c_NameOf__COL_DATA).T(") = timestamp '2021-07-27 14:27:53.0001') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
    }//using db
