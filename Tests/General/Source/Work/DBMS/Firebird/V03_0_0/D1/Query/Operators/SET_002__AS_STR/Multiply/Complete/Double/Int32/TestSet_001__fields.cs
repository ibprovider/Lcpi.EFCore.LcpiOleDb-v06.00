...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D1.Query.Operators.SET_002__AS_STR.Multiply.Complete.Double.Int32{
////////////////////////////////////////////////////////////////////////////////
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (CAST((").N("t",c_NameOf__COL_DATA1).T(" * ").N_AS_DBL("t",c_NameOf__COL_DATA2).T(") AS VARCHAR(32)) = _utf8 '28.00000000000000') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (CAST((").N("t",c_NameOf__COL_DATA1).T(" * ").N_AS_DBL("t",c_NameOf__COL_DATA2).T(") AS VARCHAR(32)) = _utf8 'BLA-BLA-BLA-HO-HO-HO') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
    }//using db
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (CAST(((").N("t",c_NameOf__COL_DATA1).T(" * ").N_AS_DBL("t",c_NameOf__COL_DATA2).T(") * 1.0) AS VARCHAR(32)) = _utf8 '28.00000000000000') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
