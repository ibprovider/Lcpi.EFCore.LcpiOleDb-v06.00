 ...
 public static void Test_001__value()
 {
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    {
     const T_DATA c_data="2021-06-01 09:12:34.5239";

     System.Int64 testID=Helper__InsertRow(db,c_data);

     var recs=db.testTable.Where(r => ((T_TEST)(object)r.DATA).Millisecond==523 && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (CAST(TRUNC(EXTRACT(MILLISECOND FROM ").N_AS_TS("t",c_NameOf__COL_DATA).T(")) AS INTEGER) = 523) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
