...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D1.Query.Funcs.DateTime.SET001.EXT.AddYears{
////////////////////////////////////////////////////////////////////////////////
//using
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var sqlt
      =new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_INTEGER).T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (DATEADD(YEAR,CAST(((0 - ").N("t",c_NameOf__COL_INTEGER).T(") - 0.5) AS INTEGER),").N("t",c_NameOf__COL_DATA).T(") = ").P_TS("__Exec_0").T(") AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")");
