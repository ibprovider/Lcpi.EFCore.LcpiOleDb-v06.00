  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var vv=new T_DATA_U(15,29,56);

     var recs=db.testTable.Where(r => r.DATA.AddMinutes(r.AMOUNT)==vv && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (DATEADD(MILLISECOND,60000 * ").N("t",c_NameOf__COL_AMOUNT).T(",").N("t",c_NameOf__COL_DATA).T(") = ").P("__vv_0").T(") AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")"));
    }//using db
