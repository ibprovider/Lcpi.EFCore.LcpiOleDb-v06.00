    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (").PUSH().N("t",c_NameOf__COL_DATA1).T(" / ").N_AS_FLT("t",c_NameOf__COL_DATA2).W_BRKTS().W_CastAsVCH(c_RESULT_VCH_LEN).POP().T(" = _utf8 'BLA-BLA-BLA') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
    }//using db
