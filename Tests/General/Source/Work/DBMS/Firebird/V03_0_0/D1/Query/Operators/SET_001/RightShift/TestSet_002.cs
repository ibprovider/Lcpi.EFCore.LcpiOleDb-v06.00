...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D1.Query.Operators.SET_001.RightShift{
////////////////////////////////////////////////////////////////////////////////

/*******************************************************************************

EXECUTE BLOCK
RETURNS (N INTEGER, SHR BIGINT)
AS
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var sqlt
      =new TestSqlTemplate()
       ...
       .T("WHERE (BIN_SHR(").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).T(") = 2) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")");
 ...
 public static void Test_002()
 {
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     const T_DATA1 c_value1=2048;
     const T_DATA2 c_value2=64+1;
...
#pragma warning disable CS0652
     var recs=db.testTable.Where(r => (r.COL_DATA1>>r.COL_DATA2)==1024 && r.TEST_ID==testID); // <--- ACHTUNG!
#pragma warning restore CS0652
     ...
     var sqlt
      =new TestSqlTemplate()
       .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
       .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
       .T("WHERE (BIN_SHR(").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).T(") = 1024) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")");
