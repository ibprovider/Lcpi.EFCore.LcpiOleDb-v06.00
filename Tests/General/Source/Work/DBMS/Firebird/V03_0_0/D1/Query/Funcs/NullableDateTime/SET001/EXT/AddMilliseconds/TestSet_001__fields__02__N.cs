  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => r.DATA.AddMilliseconds((T_AMOUNT)1)==new T_DATA_U(2021,07,26,21,49,55) && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (DATEADD(MILLISECOND,1.0,").N("t",c_NameOf__COL_DATA).T(") = timestamp '2021-07-26 21:49:55') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
    }//using db
