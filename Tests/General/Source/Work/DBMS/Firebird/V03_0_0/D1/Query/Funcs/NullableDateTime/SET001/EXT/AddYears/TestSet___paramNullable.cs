...
//                                      IBProvider and Contributors. 01.11.2018.
//
// <nullable_param>.AddYears(...)
//
using System;
using System.Linq;
...
public static class TestSet___paramNullable
{
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => vv_src.AddYears(null)==null && r.TEST_ID==testID);
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => vv_src.AddYears(vv)==null && r.TEST_ID==testID);
 ...
 //       .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_INTEGER).T(", ").N("t",c_NameOf__COL_DATA).EOL()
 //       .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
 //       .T("WHERE (DATEADD(YEAR,").P("__Exec_1").T(" / ").N("t",c_NameOf__COL_INTEGER).T(",").P("__vv_src_0").T(") = timestamp '2022-10-09') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")")
 //       .BuildSql(cn));
 // 
 //    tr.Rollback();
 //   }//using tr
