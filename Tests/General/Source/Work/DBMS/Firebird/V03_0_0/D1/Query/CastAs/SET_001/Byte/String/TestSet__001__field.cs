...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D1.Query.CastAs.SET_001.Byte.String{
////////////////////////////////////////////////////////////////////////////////

using T_SOURCE_VALUE=System.Byte;
using T_TARGET_VALUE=System.String;

using T_SOURCE_VALUE_U=System.Byte;
...
public static class TestSet__001__field
{
 ...
 private const int c_TARGET_VCH_LENGTH=3;
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (T_TARGET_VALUE)(object)r.COL_SOURCE=="255" && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_SOURCE).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (").N_AS_VCH("t",c_NameOf__COL_SOURCE,c_TARGET_VCH_LENGTH).T(" = _utf8 '255') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
