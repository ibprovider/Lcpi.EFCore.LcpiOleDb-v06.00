...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D1.Query.Funcs.Querable.SET_001.STD.Sum.String_Length{
////////////////////////////////////////////////////////////////////////////////
//using
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT CAST(TRUNC(COALESCE(SUM(CHARACTER_LENGTH(").N("t",c_NameOf__COL_DATA).T(")), 0.0)) AS INTEGER)").EOL()
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT CAST(TRUNC(COALESCE(SUM(CHARACTER_LENGTH(").N("t",c_NameOf__COL_DATA).T(")), 0.0)) AS "+xEFCore.Basement.EF.Dbms.Firebird.V03_0_0.FB_V03_0_0__Consts.c_TypeOf_BIGINT_D1+")").EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (").N("t","TEST_ID").T(" >= ").P_ID("__startTestID_0").T(") AND (").N("t","TEST_ID").T(" < ").P_ID("__curTestID_1").T(")"));
    }//using db
