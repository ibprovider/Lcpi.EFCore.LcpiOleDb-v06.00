...
//
// (bool)string_literal==bool_column
//
using System;
...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D2.Query.CastAs.SET_001.String.Boolean{
////////////////////////////////////////////////////////////////////////////////
 ...
 public static void Test_001__TRUE()
 {
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    {
     const T_SOURCE_VALUE c_valueSource="TRUE";
     const T_TARGET_VALUE c_valueTarget=true;

     System.Int64? testID=Helper__InsertRow(db,c_valueSource,c_valueTarget);

     var recs=db.testTable.Where(r => (T_TARGET_VALUE)(object)"TRUE"==r.COL_TARGET && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE ").N("t",c_NameOf__COL_TARGET).T(" AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
 ...
 public static void Test_002__FALSE()
 {
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     const T_SOURCE_VALUE c_valueSource="FALSE";
     const T_TARGET_VALUE c_valueTarget=false;

     System.Int64? testID=Helper__InsertRow(db,c_valueSource,c_valueTarget);

     var recs=db.testTable.Where(r => (T_TARGET_VALUE)(object)"FALSE"==r.COL_TARGET && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_SOURCE).T(", ").N("t",c_NameOf__COL_TARGET).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE NOT ").N("t",c_NameOf__COL_TARGET).T(" AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
