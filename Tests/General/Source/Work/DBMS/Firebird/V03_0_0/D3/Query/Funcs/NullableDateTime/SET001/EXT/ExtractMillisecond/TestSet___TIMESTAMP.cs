 ...
 public static void Test___01___ByDBMS()
 {
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    {
     T_DATA_U c_data=new T_DATA_U(2018,10,9,13,14,15,543).AddTicks(9*1000);
     ...
     var recs=db.testTable.Where(r => r.DATA.ExtractMillisecond()==543 && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (CAST(TRUNC(EXTRACT(MILLISECOND FROM ").N("t",c_NameOf__COL_DATA).T(")) AS INTEGER) = 543) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
 ...
 public static void Test___02___ByDBMS()
 {
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     T_DATA_U c_data=new T_DATA_U(2018,10,9,13,14,15,543);

     System.Int64? testID=Helper__InsertRow(db,c_data);

     var recs=db.testTable.Where(r => r.DATA.ExtractMillisecond()!=null && r.TEST_ID==testID);
