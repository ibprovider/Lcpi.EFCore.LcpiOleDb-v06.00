  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => r.DATA.AddHours((T_AMOUNT)1)==new T_DATA_U(15,29,56) && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (DATEADD(MILLISECOND,3600000 * 1.0,").N("t",c_NameOf__COL_DATA).T(") = time '15:29:56.0000') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
    }//using db
