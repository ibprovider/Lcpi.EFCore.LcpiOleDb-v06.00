...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.Operators.SET_002__AS_STR.Multiply.Complete.NUMERIC_2_0.NUMERIC_2_0{
////////////////////////////////////////////////////////////////////////////////
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (").PUSH().N("t",c_NameOf__COL_DATA1).T(" * ").N("t",c_NameOf__COL_DATA2).W_BRKTS().W_CastAsVCH(32).POP().T(" = _utf8 '28') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (").PUSH().N("t",c_NameOf__COL_DATA1).T(" * ").N("t",c_NameOf__COL_DATA2).W_BRKTS().T(" * 1").W_BRKTS().W_CastAsVCH(32).POP().T(" = _utf8 '28') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
