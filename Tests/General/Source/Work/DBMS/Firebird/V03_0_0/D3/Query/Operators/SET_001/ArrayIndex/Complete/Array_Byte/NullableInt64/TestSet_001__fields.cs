...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.Operators.SET_001.ArrayIndex.Complete.Array_Byte.NullableInt64{
////////////////////////////////////////////////////////////////////////////////
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (CASE").EOL()
        .T("    WHEN COALESCE((0 <= ").N_AS_INT("t",c_NameOf__COL_DATA2).T(") AND (").N_AS_INT("t",c_NameOf__COL_DATA2).T(" < OCTET_LENGTH(").N("t",c_NameOf__COL_DATA1).T(")), TRUE) THEN ASCII_VAL(SUBSTRING(").N("t",c_NameOf__COL_DATA1).T(" FROM ").N_AS_INT("t",c_NameOf__COL_DATA2).T(" + 1 FOR 1))").EOL()
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => 40==r.COL_DATA1[(T_DATA2_U)r.COL_DATA2] && r.TEST_ID==testID);
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     for(T_DATA2_U iByte=0;iByte!=bytes.Length;++iByte)
     {
      ...
      var recs=db.testTable.Where(r => r.COL_DATA1[(T_DATA2_U)r.COL_DATA2]==expectedByte && r.TEST_ID==testID);
