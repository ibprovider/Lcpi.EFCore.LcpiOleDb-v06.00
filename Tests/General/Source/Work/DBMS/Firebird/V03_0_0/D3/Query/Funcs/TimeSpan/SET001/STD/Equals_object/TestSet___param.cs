...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.Funcs.TimeSpan.SET001.STD.Equals__object{
////////////////////////////////////////////////////////////////////////////////
//using
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     const long c_ticks=23*c_ticks_in_hour;

     var recs=db.testTable.Where(r => vv_src.Equals((object)(r.DATA+new System.TimeSpan(c_ticks)-new System.TimeSpan(c_ticks))) && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (").P_TIMESPAN("__vv_src_0").T(" = ((").N("t",c_NameOf__COL_DATA).T(" + CAST(82800.0000 AS NUMERIC(18,4))) - CAST(82800.0000 AS NUMERIC(18,4)))) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")"));
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (").P_TIMESPAN("__vv_src_0").T(" = (").N("t",c_NameOf__COL_DATA).T(" + CAST(0.0000 AS NUMERIC(18,4)))) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")"));
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (").P_TIMESPAN("__vv_src_0").T(" = ((").N("t",c_NameOf__COL_DATA).T(" + CAST(0.0000 AS NUMERIC(18,4))) + CAST(0.0000 AS NUMERIC(18,4)))) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")"));
