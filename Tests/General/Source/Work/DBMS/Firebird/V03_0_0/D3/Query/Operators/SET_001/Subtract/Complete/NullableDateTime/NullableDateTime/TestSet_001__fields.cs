...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.Operators.SET_001.Subtract.Complete.NullableDateTime.NullableDateTime{
////////////////////////////////////////////////////////////////////////////////

using T_DATA1   =System.Nullable<System.DateTime>;
using T_DATA2   =System.Nullable<System.DateTime>;
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (r.COL_DATA1-r.COL_DATA2)==(T_RESULT)(new T_RESULT_U(1,13,19,4,119)) && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE ((DATEDIFF(MILLISECOND,").N("t",c_NameOf__COL_DATA2).T(",").N("t",c_NameOf__COL_DATA1).T(") * 0.001) = CAST(134344.1190 AS NUMERIC(18,4))) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (r.COL_DATA1-r.COL_DATA2)==(T_RESULT)(new T_RESULT_U()-new T_RESULT_U(1,13,19,4,119)) && r.TEST_ID==testID);
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (r.COL_DATA1-r.COL_DATA2)==((T_RESULT)new T_RESULT_U(1,13,10,7,0)+new T_RESULT_U(9999000)) && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE ((DATEDIFF(MILLISECOND,").N("t",c_NameOf__COL_DATA2).T(",").N("t",c_NameOf__COL_DATA1).T(") * 0.001) = CAST(133807.9999 AS NUMERIC(18,4))) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
