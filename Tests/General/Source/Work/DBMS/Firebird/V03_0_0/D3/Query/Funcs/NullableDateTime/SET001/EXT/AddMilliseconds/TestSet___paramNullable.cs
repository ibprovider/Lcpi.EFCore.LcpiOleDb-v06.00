    ...
    using(var db=new MyContext(tr))
    {
     ...
     var sqlt
      =new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_INTEGER).T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (DATEADD(MILLISECOND,CAST(").N("t",c_NameOf__COL_INTEGER).T(" AS DOUBLE PRECISION) / CAST((2 * ").N("t",c_NameOf__COL_INTEGER).T(") AS DOUBLE PRECISION),").P_TS("__vv_src_0").T(") = timestamp '2018-10-09 00:00:00.0005') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")");
