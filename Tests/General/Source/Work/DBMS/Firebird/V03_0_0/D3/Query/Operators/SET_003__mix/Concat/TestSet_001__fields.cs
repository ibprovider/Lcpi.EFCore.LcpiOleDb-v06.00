...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.Operators.SET_001__mix.Concat{
////////////////////////////////////////////////////////////////////////////////
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE ((COALESCE(").N("t",c_NameOf__COL_DATA1).T(", _utf8 '-')||COALESCE(").N("t",c_NameOf__COL_DATA2).T(", _utf8 '-')) = _utf8 '12') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => ((r.COL_DATA1??r.COL_DATA2)+(r.COL_DATA2??r.COL_DATA1))=="12" && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE ((COALESCE(").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).T(", _utf8 '')||COALESCE(").N("t",c_NameOf__COL_DATA2).T(", ").N("t",c_NameOf__COL_DATA1).T(", _utf8 '')) = _utf8 '12') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
