...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.Operators.SET_002__AS_STR.Subtract.Complete.Int32.Int16{
////////////////////////////////////////////////////////////////////////////////
...
public static class TestSet_001__fields
{
 ...
 //-----------------------------------------------------------------------
 [Test]
 public static void Test_MM001__max_min()
 {
  using(var cn=LocalCnHelper.CreateCn())
  {
   cn.Open();
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (string)(object)(r.COL_DATA1-r.COL_DATA2)=="2147516415" && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (").PUSH().N("t",c_NameOf__COL_DATA1).T(" - ").N("t",c_NameOf__COL_DATA2).W_BRKTS().W_CastAsVCH(20).POP().T(" = _utf8 '2147516415') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
 ...
 //-----------------------------------------------------------------------
 [Test]
 public static void Test_MM002__min_max()
 {
  using(var cn=LocalCnHelper.CreateCn())
  {
   cn.Open();
