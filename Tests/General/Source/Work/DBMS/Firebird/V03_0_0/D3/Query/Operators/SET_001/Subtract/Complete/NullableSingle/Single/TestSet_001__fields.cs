...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.Operators.SET_001.Subtract.Complete.NullableSingle.Single{
////////////////////////////////////////////////////////////////////////////////
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var sqlt
      =new TestSqlTemplate()
       ...
       .T("WHERE ((").N("t",c_NameOf__COL_DATA1).T(" - ").N("t",c_NameOf__COL_DATA2).T(") = 3.0) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")");
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (r.COL_DATA1-(r.COL_DATA2-r.COL_DATA2))==7 && r.TEST_ID==testID);
     ...
     var sqlt
      =new TestSqlTemplate()
       .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
       .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
       .T("WHERE ((").N("t",c_NameOf__COL_DATA1).T(" - (").N("t",c_NameOf__COL_DATA2).T(" - ").N("t",c_NameOf__COL_DATA2).T(")) = 7.0) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")");
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var sqlt
      =new TestSqlTemplate()
       .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
       .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
       .T("WHERE (((").N("t",c_NameOf__COL_DATA1).T(" - ").N("t",c_NameOf__COL_DATA2).T(") - ").N("t",c_NameOf__COL_DATA2).T(") = -1.0) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")");
