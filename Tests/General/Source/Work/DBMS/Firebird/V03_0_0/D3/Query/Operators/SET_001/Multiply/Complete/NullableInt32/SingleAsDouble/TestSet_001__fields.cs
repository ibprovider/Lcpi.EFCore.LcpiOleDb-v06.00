...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.Operators.SET_001.Multiply.Complete.NullableInt32.SingleAsDouble{
////////////////////////////////////////////////////////////////////////////////
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var sqlt
      =new TestSqlTemplate()
       .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
       .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
       .T("WHERE (((CAST(").N("t",c_NameOf__COL_DATA1).T(" AS DOUBLE PRECISION) * ").N("t",c_NameOf__COL_DATA2).T(") * ").N("t",c_NameOf__COL_DATA2).T(") = 112.0) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")");
