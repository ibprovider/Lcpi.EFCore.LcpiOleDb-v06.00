  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => System.Math.Abs((double)r.DATA)==123 && r.TEST_ID==testID);
     ...
     var sqlt
      =new TestSqlTemplate()
        ...
        .T("WHERE (ABS(CAST(").N("t",c_NameOf__COL_DATA).T(" AS DOUBLE PRECISION)) = 123.0) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")");
