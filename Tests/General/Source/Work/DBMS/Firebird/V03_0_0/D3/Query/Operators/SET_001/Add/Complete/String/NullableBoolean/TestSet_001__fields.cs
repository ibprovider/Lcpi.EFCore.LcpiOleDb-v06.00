  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (r.COL_DATA1+r.COL_DATA2)=="asd--FALSE" && r.TEST_ID==testID);
     ...
     var sqlt
      =new TestSqlTemplate()
       ...
       .T("WHERE ((").N_TXT_NP("t",c_NameOf__COL_DATA1).T("||").N_TXT_NP("t",c_NameOf__COL_DATA2).T(") = _utf8 'asd--FALSE') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")");
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (r.COL_DATA1+(string)(object)r.COL_DATA2)=="asd--TRUE" && r.TEST_ID==testID);
     ...
     var sqlt
      =new TestSqlTemplate()
       .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
       .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
       .T("WHERE ((").N_TXT_NP("t",c_NameOf__COL_DATA1).T("||").N_TXT_NP("t",c_NameOf__COL_DATA2).T(") = _utf8 'asd--TRUE') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")");
