 ...
 public static void Test___text___ByEF()
 {
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    {
     System.Int64 testID=Helper__InsertRow(db,"QQ");

     T_DATA vv="0123456789012345678901234567890123456789_ABC\0123_012345678901234567890123456789";

#pragma warning disable CS0472
     var recs=db.testTable.Where(r => (r.DATA+vv).Contains("ABC\0123") && r.TEST_ID==testID);
#pragma warning restore CS0472
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (POSITION(_utf8 'ABC'||_utf8 x'00'||_utf8 '123', ").N_TXT_NP("t",c_NameOf__COL_DATA).T("||").P_TXT("__vv_0").T(") > 0) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")"));
...
#pragma warning disable CS0472
     var recs=db.testTable.Where(r => (r.DATA+vv).Contains(vv) && r.TEST_ID==testID);
#pragma warning restore CS0472
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (POSITION(").P_TXT("__vv_0").T(", ").N_TXT_NP("t",c_NameOf__COL_DATA).T("||").P_TXT("__vv_0").T(") > 0) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")"));
