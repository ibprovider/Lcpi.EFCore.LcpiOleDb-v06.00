    ...
    using(var db=new MyContext(tr))
    {
     ...
     var sqlt
      =new TestSqlTemplate()
        ...
        .T("WHERE (TRUNC(").N("n",c_NameOf__COL_DATA).T(") = 1) AND (").N("n","TEST_ID").T(" = ").P_ID("__testID_0").T(")");
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var sqlt
      =new TestSqlTemplate()
        .T("SELECT ").N("n","TEST_ID").T(", ").N("n",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("n").EOL()
        .T("WHERE (TRUNC(2 * TRUNC(").N("n",c_NameOf__COL_DATA).T(")) = 2) AND (").N("n","TEST_ID").T(" = ").P_ID("__testID_0").T(")");
