    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (RIGHT(").N_TXT_NP("t",c_NameOf__COL_DATA).T("||_utf8 '', CHARACTER_LENGTH(").N("t",c_NameOf__COL_DATA).T(")) = ").N("t",c_NameOf__COL_DATA).T(") AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (r.DATA+"45").EndsWith("12345") && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (RIGHT(").N_TXT_NP("t",c_NameOf__COL_DATA).T("||_utf8 '45', 5) = _utf8 '12345') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (r.DATA+"45").EndsWith("\012345") && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (RIGHT(").N_TXT_NP("t",c_NameOf__COL_DATA).T("||_utf8 '45', 6) = _utf8 x'00'||_utf8 '12345') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (RIGHT(").N_TXT_NP("t",c_NameOf__COL_DATA).T("||_utf8 '-', 8) = _utf8 x'00'||_utf8 'ABC1234') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
    }//using db
