  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     Assert.AreEqual
      (new System.TimeSpan(17,14,7)+new System.TimeSpan(1000),
       c_value1+c_value2);

     var recs=db.testTable.Where(r => (r.COL_DATA1+r.COL_DATA2)==((T_RESULT)new T_RESULT_U(17,14,7)+new T_RESULT_U(1000)) && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE ((").N("t",c_NameOf__COL_DATA1).T(" + ").N("t",c_NameOf__COL_DATA2).T(") = CAST(62047.0001 AS NUMERIC(18,4))) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
