...
//                                      IBProvider and Contributors. 19.09.2018.
using System;
using System.Linq;
...
public static class TestSet___000
{
 ...
 {
  Assert.IsTrue
   ("ASD\0123".StartsWith('A'));

  Assert.IsFalse
   ("ASD\0123".StartsWith('\0'));

  Assert.IsTrue
   ("\0ASD123".StartsWith('\0'));

  Assert.IsFalse
   ("\0ASD123".StartsWith('A'));
 }//Test_00
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => r.DATA.StartsWith('B') && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (").N("t",c_NameOf__COL_DATA).T(" STARTING WITH _utf8 'B') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
    }//using db
