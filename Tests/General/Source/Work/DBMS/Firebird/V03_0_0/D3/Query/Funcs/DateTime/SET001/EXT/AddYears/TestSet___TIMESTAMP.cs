...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.Funcs.DateTime.SET001.EXT.AddYears{
////////////////////////////////////////////////////////////////////////////////
//using
...
public static class TestSet___TIMESTAMP
{
 ...
 //-----------------------------------------------------------------------
 [Test]
 public static void Test___expr7___ByDBMS()
 {
  using(var cn=LocalCnHelper.CreateCn())
  {
   cn.Open();
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => r.DATA.AddYears(xdbfunc.DbCast.AsInteger(r.COL_INTEGER+0.5))==c_data.AddYears(xdbfunc.DbCast.AsInteger(1.5)) && r.TEST_ID==testID);
     ...
     var sqlt
      =new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_INTEGER).T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (DATEADD(YEAR,CAST((CAST(").N("t",c_NameOf__COL_INTEGER).T(" AS DOUBLE PRECISION) + 0.5) AS INTEGER),").N("t",c_NameOf__COL_DATA).T(") = ").P_TS("__Exec_0").T(") AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")");
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var sqlt
      =new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_INTEGER).T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (DATEADD(YEAR,CAST((CAST((0 - ").N("t",c_NameOf__COL_INTEGER).T(") AS DOUBLE PRECISION) - 0.5) AS INTEGER),").N("t",c_NameOf__COL_DATA).T(") = ").P_TS("__Exec_0").T(") AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")");
