...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.Operators.SET_002__AS_STR.Add.Complete.Int16.Single{
////////////////////////////////////////////////////////////////////////////////
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (CAST((").N_AS_FLT("t",c_NameOf__COL_DATA1).T(" + ").N("t",c_NameOf__COL_DATA2).T(") AS VARCHAR(32)) = _utf8 '11.00000000000000') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (CAST(((").N_AS_FLT("t",c_NameOf__COL_DATA1).T(" + ").N("t",c_NameOf__COL_DATA2).T(") / ").N_AS_FLT("t",c_NameOf__COL_DATA1).T(") AS VARCHAR(32)) = _utf8 '1.571428571428571') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
