  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (new T_ARR1_E[]{r.COL_DATA1,r.COL_DATA2} /*OP{*/ != /*}OP*/ new T_ARR2_E[]{r.COL_DATA2,r.COL_DATA1}) && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE ((((").N("t",c_NameOf__COL_DATA1).T(" <> ").N("t",c_NameOf__COL_DATA2).T(") OR (").N("t",c_NameOf__COL_DATA1).T(" IS NULL) OR (").N("t",c_NameOf__COL_DATA2).T(" IS NULL)) AND ((").N("t",c_NameOf__COL_DATA1).T(" IS NOT NULL) OR (").N("t",c_NameOf__COL_DATA2).T(" IS NOT NULL))) OR (((").N("t",c_NameOf__COL_DATA2).T(" <> ").N("t",c_NameOf__COL_DATA1).T(") OR (").N("t",c_NameOf__COL_DATA2).T(" IS NULL) OR (").N("t",c_NameOf__COL_DATA1).T(" IS NULL)) AND ((").N("t",c_NameOf__COL_DATA2).T(" IS NOT NULL) OR (").N("t",c_NameOf__COL_DATA1).T(" IS NOT NULL)))) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
