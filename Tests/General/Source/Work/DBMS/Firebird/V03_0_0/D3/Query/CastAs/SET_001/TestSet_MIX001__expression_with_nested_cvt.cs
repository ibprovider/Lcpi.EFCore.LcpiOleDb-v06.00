...
////////////////////////////////////////////////////////////////////////////////
//class TestSet_MIX001__expression_with_nested_cvt

public static class TestSet_MIX001__expression_with_nested_cvt
{
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => ((T_TARGET_VALUE)(object)(r.COL_SOURCE-(T_TARGET_VALUE)(object)r.COL_SOURCE))==(T_TARGET_VALUE)(object)(c_valueSource-(T_TARGET_VALUE)(object)c_valueSource) && r.TEST_ID==testID);
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_SOURCE).T(", ").N("t",c_NameOf__COL_TARGET).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (CAST(TRUNC(").N("t",c_NameOf__COL_SOURCE).T(" - CAST(CAST(TRUNC(").N("t",c_NameOf__COL_SOURCE).T(") AS "+c_NameOf__TARGET_SQL_TYPE+") AS DOUBLE PRECISION)) AS "+c_NameOf__TARGET_SQL_TYPE+") = ").P_I2("__p_0").T(") AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")"));
