    ...
    using(var db=new MyContext(tr))
    {
     ...
     var sqlt
      =new TestSqlTemplate()
        .T("SELECT ").N("n","TEST_ID").T(", ").N("n",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("n").EOL()
        .T("WHERE (ROUND(ROUND(").N("n",c_NameOf__COL_DATA).T(", 0), 0) = 2) AND (").N("n","TEST_ID").T(" = ").P_ID("__testID_0").T(")");
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => xdbfunc.DbMath.Round(2*xdbfunc.DbMath.Round(r.DATA,0))==4 && r.TEST_ID==testID);
     ...
     var sqlt
      =new TestSqlTemplate()
        .T("SELECT ").N("n","TEST_ID").T(", ").N("n",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("n").EOL()
        .T("WHERE (ROUND(2 * ROUND(").N("n",c_NameOf__COL_DATA).T(", 0)) = 4) AND (").N("n","TEST_ID").T(" = ").P_ID("__testID_0").T(")");
