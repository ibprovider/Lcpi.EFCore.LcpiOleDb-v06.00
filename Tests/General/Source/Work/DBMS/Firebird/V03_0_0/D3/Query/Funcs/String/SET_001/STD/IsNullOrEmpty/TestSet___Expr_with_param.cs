...
public static class TestSet___Expr_with_param
{
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => string.IsNullOrEmpty(r.DATA+vv) && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (OCTET_LENGTH(").N_TXT_NP("t",c_NameOf__COL_DATA).T("||").P_TXT("__vv_0").T(") = 0) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")"));
    }//using db
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (OCTET_LENGTH(").N_TXT_NP("t",c_NameOf__COL_DATA).T("||_utf8 '') = 0) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")"));
    }//using db
 ...
 //-----------------------------------------------------------------------
 [Test]
 public static void Test___concatFieldAndParamE___ByEF()
 {
  using(var cn=LocalCnHelper.CreateCn())
  {
   cn.Open();
 ...
 }//Test___concatFieldAndParamE___ByEF

 //-----------------------------------------------------------------------
 [Test]
 public static void Test___concatFieldAndParamZ___ByEF()
 {
  using(var cn=LocalCnHelper.CreateCn())
  {
   cn.Open();
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => string.IsNullOrEmpty(r.DATA.Replace(vv1,vv2)) && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE ((").N("t",c_NameOf__COL_DATA).IS_NULL().T(") OR (OCTET_LENGTH(REPLACE(").N("t",c_NameOf__COL_DATA).T(", ").P_TXT("__vv1_0").T(", ").P_TXT("__vv2_1").T(")) = 0)) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_2").T(")"));
