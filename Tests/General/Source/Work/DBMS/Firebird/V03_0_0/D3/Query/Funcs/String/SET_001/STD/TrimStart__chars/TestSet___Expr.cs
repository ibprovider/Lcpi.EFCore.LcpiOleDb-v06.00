  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (r.DATA+" ABC ").TrimStart(null)=="123  ABC " && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (TRIM(LEADING FROM ").N_TXT_NP("t",c_NameOf__COL_DATA).T("||_utf8 ' ABC ') = _utf8 '123  ABC ') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (r.DATA+" ABC ").TrimStart(new char[0])=="123  ABC " && r.TEST_ID==testID);
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (r.DATA+"--ABC-").TrimStart(new char[]{'-'})=="123----ABC-" && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (TRIM(LEADING _utf8 '-' FROM ").N_TXT_NP("t",c_NameOf__COL_DATA).T("||_utf8 '--ABC-') = _utf8 '123----ABC-') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (r.DATA+"\0A\0").TrimStart(new char[]{'\0'})=="123\0\0\0\0A\0" && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (TRIM(LEADING _utf8 x'00' FROM ").N_TXT_NP("t",c_NameOf__COL_DATA).T("||_utf8 x'00'||_utf8 'A'||_utf8 x'00') = _utf8 '123'||_utf8 x'00000000'||_utf8 'A'||_utf8 x'00') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (r.DATA+"\0-ABC\0-").TrimStart(new char[]{'\0','-'})=="123\0-\0-ABC\0-" && r.TEST_ID==testID);
