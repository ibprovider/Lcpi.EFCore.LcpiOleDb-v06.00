...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.Funcs.NullableDateOnly.SET001.STD.Value{
////////////////////////////////////////////////////////////////////////////////
//using
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(") AND (COALESCE(").N("t",c_NameOf__COL_DATA).T(", CAST(_utf8 'TRAP FOR NULL' AS DATE)) = ").P_DE("__c_data_1").T(")"));
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(") AND (COALESCE(").N("t",c_NameOf__COL_DATA).T(", CAST(_utf8 'TRAP FOR NULL' AS DATE)) = date '2020-11-08')"));
    }//using db
