  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (r.DATA+" Q  ").Trim()=="123  Q" && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (TRIM(").N_TXT_NP("t",c_NameOf__COL_DATA).T("||_utf8 ' Q  ') = _utf8 '123  Q') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (r.DATA+" QWE ").Trim()=="1\03  QWE " && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (TRIM(").N_TXT_NP("t",c_NameOf__COL_DATA).T("||_utf8 ' QWE ') = _utf8 '1'||_utf8 x'00'||_utf8 '3  QWE ') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
