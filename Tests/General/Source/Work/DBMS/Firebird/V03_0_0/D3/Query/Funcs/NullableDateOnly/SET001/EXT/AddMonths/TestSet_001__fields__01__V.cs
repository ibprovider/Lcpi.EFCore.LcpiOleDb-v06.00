...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.Funcs.NullableDateOnly.SET001.EXT.AddMonths{
////////////////////////////////////////////////////////////////////////////////
//using
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     T_DATA vv=new T_DATA_U(2018,12,9);

     var recs=db.testTable.Where(r => r.DATA.AddMonths(2)==vv && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (DATEADD(MONTH,2,").N("t",c_NameOf__COL_DATA).T(") = ").P_DE("__vv_0").T(") AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")"));
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => r.DATA.AddMonths(3)==r.DATA.AddMonths(3) && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE ((DATEADD(MONTH,3,").N("t",c_NameOf__COL_DATA).T(") = DATEADD(MONTH,3,").N("t",c_NameOf__COL_DATA).T(")) OR (").N("t",c_NameOf__COL_DATA).IS_NULL().T(")) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
