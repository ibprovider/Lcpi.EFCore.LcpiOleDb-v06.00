...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.Funcs.DateTime.SET001.STD.Equals__obj{
////////////////////////////////////////////////////////////////////////////////
//using
...
public static class TestSet___param
{
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => vv_src.Equals((object)new System.DateTime(2018,10,9)) && r.TEST_ID==testID);
 ...
 //-----------------------------------------------------------------------
 [Test]
 public static void Test___expr2b___ByEF()
 {
  using(var cn=LocalCnHelper.CreateCn())
  {
   cn.Open();
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => vv_src.Equals((object)(r.DATA+new System.TimeSpan(0)+new System.TimeSpan(0))) && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (").P_TS("__vv_src_0").T(" = DATEADD(MILLISECOND,1000 * CAST(0.0000 AS NUMERIC(18,4)),DATEADD(MILLISECOND,1000 * CAST(0.0000 AS NUMERIC(18,4)),").N("t",c_NameOf__COL_DATA).T("))) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")"));
 ...
 //-----------------------------------------------------------------------
 [Test]
 public static void Test___expr3b___ByEF()
 {
  using(var cn=LocalCnHelper.CreateCn())
  {
   cn.Open();
