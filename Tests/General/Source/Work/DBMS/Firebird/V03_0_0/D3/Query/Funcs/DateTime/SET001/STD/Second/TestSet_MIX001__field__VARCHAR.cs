 ...
 public static void Test_001__value()
 {
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    {
     const T_DATA c_data="2021-06-01 09:12:34.5234";

     System.Int64 testID=Helper__InsertRow(db,c_data);

     var recs=db.testTable.Where(r => ((T_TEST)(object)r.DATA).Second==34 && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (CAST(TRUNC(EXTRACT(SECOND FROM ").N_AS_TS("t",c_NameOf__COL_DATA).T(")) AS INTEGER) = 34) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
