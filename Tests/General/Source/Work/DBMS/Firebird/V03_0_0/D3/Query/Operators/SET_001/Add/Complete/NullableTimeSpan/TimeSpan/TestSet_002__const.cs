...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.Operators.SET_001.Add.Complete.NullableTimeSpan.TimeSpan{
////////////////////////////////////////////////////////////////////////////////
    ...
    using(var db=new MyContext(tr))
    {
     ...
     Assert.AreEqual
      (new System.TimeSpan(17,14,7),
       c_value1+new System.TimeSpan(5,3,59));

     var recs=db.testTable.Where(r => (r.COL_DATA1+(T_DATA2)new T_DATA2_U(5,3,59))==((T_RESULT)new T_RESULT_U(17,14,7)) && r.TEST_ID==testID);
     ...
     var sqlt
      =new TestSqlTemplate()
       ...
       .T("WHERE ((").N("t",c_NameOf__COL_DATA1).T(" + CAST(18239.0000 AS NUMERIC(18,4))) = CAST(62047.0000 AS NUMERIC(18,4))) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")");
 ...
 public static void Test_002__ms()
 {
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     T_DATA1 c_value1=new T_DATA1_U(1,2,8);
     T_DATA2 c_value2=new T_DATA2_U(14,12,11);

     System.Int64? testID=Helper__InsertRow(db,c_value1,c_value2);

     Assert.AreEqual
      (new System.TimeSpan(0,15,14,19,13),
       c_value1+new System.TimeSpan(0,14,12,11,13));

     var recs=db.testTable.Where(r => (r.COL_DATA1+(T_DATA2)new T_DATA2_U(0,14,12,11,13))==((T_RESULT)new T_RESULT_U(0,15,14,19,13)) && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE ((").N("t",c_NameOf__COL_DATA1).T(" + CAST(51131.0130 AS NUMERIC(18,4))) = CAST(54859.0130 AS NUMERIC(18,4))) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
 ...
 public static void Test_003__mcs()
 {
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    {
     T_DATA1 c_value1=new T_DATA1_U(1,2,8);
     T_DATA2 c_value2=new System.TimeSpan(14,12,11)+new System.TimeSpan(1000);

     System.Int64? testID=Helper__InsertRow(db,c_value1,c_value2);

     Assert.AreEqual
      (new System.TimeSpan(15,14,19)+new System.TimeSpan(1000),
       c_value1+(new System.TimeSpan(14,12,11)+new System.TimeSpan(1000)));

     var recs=db.testTable.Where(r => (r.COL_DATA1+(T_DATA2)(new T_DATA2_U(0,14,12,11)+new T_DATA2_U(1000)))==((T_RESULT)new T_RESULT_U(15,14,19)+new T_RESULT_U(1000)) && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE ((").N("t",c_NameOf__COL_DATA1).T(" + CAST(51131.0001 AS NUMERIC(18,4))) = CAST(54859.0001 AS NUMERIC(18,4))) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
