...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.Operators.SET_002__AS_STR.Add.Complete.DateTime.NullableTimeSpan{
////////////////////////////////////////////////////////////////////////////////
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     T_DATA2 c_value2=new T_DATA2_U(0,2,3,1,432).Add(new System.TimeSpan(300));
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (string)(object)(r.COL_DATA1+r.COL_DATA2)=="2021-02-02 14:30:33.4321" && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (CAST(DATEADD(MILLISECOND,1000 * ").N("t",c_NameOf__COL_DATA2).T(",").N("t",c_NameOf__COL_DATA1).T(") AS VARCHAR(24)) = _utf8 '2021-02-02 14:30:33.4321') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
