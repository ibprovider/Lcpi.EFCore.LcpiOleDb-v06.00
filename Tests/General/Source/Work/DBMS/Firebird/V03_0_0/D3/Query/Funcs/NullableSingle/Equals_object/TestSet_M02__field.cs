...
////////////////////////////////////////////////////////////////////////////////
//class TestSet_M02__field

public static class TestSet_M02__field
{
     ...
     new tagData
      (4324,
       new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE ((").N("t",c_NameOf__COL_DATA).T(" <> ").P_I4("__testData_ParamValue_0").T(") OR (").N("t",c_NameOf__COL_DATA).IS_NULL().T(")) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")")),
     ...
     new tagData
      (4324L,
       new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE ((").N("t",c_NameOf__COL_DATA).T(" <> ").P_I8("__testData_ParamValue_0").T(") OR (").N("t",c_NameOf__COL_DATA).IS_NULL().T(")) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")")),
    };//sm_Data
  ...
  using(var cn=Helper__CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     System.Int64? testID_=Helper__InsertRow(db,c_data);
     ...
     for(int nPass=0;nPass!=3;)
     {
      ...
      for(int iTest=0;iTest!=sm_Data.Length;++iTest)
      {
       ...
       var recs=db.testTable.Where(r => !r.DATA.Equals(testData.ParamValue) && r.TEST_ID==testID);
