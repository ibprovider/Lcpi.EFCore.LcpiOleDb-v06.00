  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     string src="QWEEEEEEEEEEEEEEEEEEEEEEEEEEE";

     var recs=db.testTable.Where(r => (src+r.DATA).StartsWith('Q') && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE ((").P_TXT("__src_0").T("||").N_TXT_NP("t",c_NameOf__COL_DATA).T(") STARTING WITH _utf8 'Q') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")"));
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (src+r.DATA).StartsWith(vv) && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE ((").P_TXT("__src_0").T("||").N_TXT_NP("t",c_NameOf__COL_DATA).T(") STARTING WITH ").P_CH1("__vv_1").T(") AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_2").T(")"));
