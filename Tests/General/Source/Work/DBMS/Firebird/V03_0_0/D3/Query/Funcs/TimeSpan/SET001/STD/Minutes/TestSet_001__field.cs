  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => r.DATA.Minutes==15 && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (CAST((MOD(TRUNC(").N("t",c_NameOf__COL_DATA).T("), 3600) / 60) AS INTEGER) = 15) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
