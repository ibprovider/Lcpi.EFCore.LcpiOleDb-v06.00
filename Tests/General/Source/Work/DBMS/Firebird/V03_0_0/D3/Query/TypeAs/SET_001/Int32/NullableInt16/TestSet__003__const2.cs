...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.TypeAs.SET_001.Int32.NullableInt16{
////////////////////////////////////////////////////////////////////////////////
...
#pragma warning restore CS0458 // The result of the expression is always 'null'
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (CAST(").N("t",c_NameOf__COL_SOURCE).T(" AS "+c_NameOf__TARGET_SQL_TYPE+") = -1) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
...
#pragma warning restore CS0458 // The result of the expression is always 'null'
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_SOURCE).T(", ").N("t",c_NameOf__COL_TARGET).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (CAST(").N("t",c_NameOf__COL_SOURCE).T(" AS "+c_NameOf__TARGET_SQL_TYPE+") = 0) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
