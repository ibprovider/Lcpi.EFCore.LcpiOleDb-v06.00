...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.Operators.SET_002__AS_STR.Multiply.Complete.Single.Double{
////////////////////////////////////////////////////////////////////////////////
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (string)(object)(r.COL_DATA1*r.COL_DATA2)=="BLA-BLA-BLA-HO-HO-HO" && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (CAST((").N("t",c_NameOf__COL_DATA1).T(" * ").N("t",c_NameOf__COL_DATA2).T(") AS VARCHAR(32)) = _utf8 'BLA-BLA-BLA-HO-HO-HO') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
    }//using db
