...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.TypeAs.SET_001.Int32.String{
////////////////////////////////////////////////////////////////////////////////
...
public static class TestSet__001__field
{
 ...
 private const int c_TARGET_VCH_LENGTH=11;
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (r.COL_SOURCE as object as T_TARGET_VALUE)=="-1" && r.TEST_ID==testID);
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (r.COL_SOURCE as object as T_TARGET_VALUE)=="0" && r.TEST_ID==testID);
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (r.COL_SOURCE as object as T_TARGET_VALUE)=="-2147483648" && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_SOURCE).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (").N_AS_VCH("t",c_NameOf__COL_SOURCE,c_TARGET_VCH_LENGTH).T(" = _utf8 '-2147483648') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (r.COL_SOURCE as object as T_TARGET_VALUE)=="2147483647" && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_SOURCE).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (").N_AS_VCH("t",c_NameOf__COL_SOURCE,c_TARGET_VCH_LENGTH).T(" = _utf8 '2147483647') AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
