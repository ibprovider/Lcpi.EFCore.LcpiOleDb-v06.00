  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => EF.Functions.Like(r.DATA1,r.DATA2,r.DATA3) && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (").N("t",c_NameOf__COL_DATA1).T(" LIKE ").N("t",c_NameOf__COL_DATA2).T(" ESCAPE ").N("t",c_NameOf__COL_DATA3).T(") AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")"));
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).T(", ").N("t",c_NameOf__COL_DATA3).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE ((_utf8 ''||").N_TXT_NP("t",c_NameOf__COL_DATA1).T(") LIKE (_utf8 ''||").N_TXT_NP("t",c_NameOf__COL_DATA2).T(") ESCAPE (_utf8 ''||").N_TXT_NP("t",c_NameOf__COL_DATA3).T(")) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_1").T(")"));
