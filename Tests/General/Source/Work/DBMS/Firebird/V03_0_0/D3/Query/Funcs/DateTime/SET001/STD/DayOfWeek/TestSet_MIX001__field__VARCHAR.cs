  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => ((T_TEST)(object)r.DATA).DayOfWeek==System.DayOfWeek.Tuesday && r.TEST_ID==testID);
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        ...
        .T("WHERE (EXTRACT(WEEKDAY FROM ").N_AS_TS("t",c_NameOf__COL_DATA).T(") = 2) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")"));
