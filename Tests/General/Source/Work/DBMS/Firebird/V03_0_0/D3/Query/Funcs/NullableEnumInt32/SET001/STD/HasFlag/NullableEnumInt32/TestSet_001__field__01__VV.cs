  ...
  using(var cn=Helper__CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => r.TEST_ID==testID && ((TestEnum001)r.COL_DATA1).HasFlag((TestEnum001)c_value2));
  ...
  using(var cn=Helper__CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => r.TEST_ID==testID && ((TestEnum001)r.COL_DATA1).HasFlag((TestEnum001)(T_DATA2)(TestEnum001.Flag1|TestEnum001.Flag2)));
