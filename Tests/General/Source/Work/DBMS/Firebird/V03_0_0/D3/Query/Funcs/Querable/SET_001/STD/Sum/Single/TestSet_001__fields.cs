...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.Funcs.Querable.SET_001.STD.Sum.Single{
////////////////////////////////////////////////////////////////////////////////
//using
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT CAST(COALESCE(SUM(").N("t",c_NameOf__COL_DATA).T("), 0.0) AS FLOAT)").EOL()
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     db.CheckTextOfLastExecutedCommand
      (new TestSqlTemplate()
        .T("SELECT COALESCE(SUM(").N("t",c_NameOf__COL_DATA).T("), 0.0)").EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
        .T("WHERE (").N("t","TEST_ID").T(" >= ").P_ID("__startTestID_0").T(") AND (").N("t","TEST_ID").T(" < ").P_ID("__curTestID_1").T(")"));
    }//using db
