...
//                                      IBProvider and Contributors. 12.04.2019.
//
...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.Funcs.DbMath.SET_001.STD.Round_scale.NullableDouble{
////////////////////////////////////////////////////////////////////////////////
//using
...
public static class TestSet___DBL
{
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => xdbfunc.DbMath.Round(xdbfunc.DbMath.Round(r.DATA,0),0)==2 && r.TEST_ID==testID);
     ...
     var sqlt
      =new TestSqlTemplate()
        .T("SELECT ").N("n","TEST_ID").T(", ").N("n",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("n").EOL()
        .T("WHERE (ROUND(ROUND(").N("n",c_NameOf__COL_DATA).T(", 0), 0) = 2.0) AND (").N("n","TEST_ID").T(" = ").P_ID("__testID_0").T(")");
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => xdbfunc.DbMath.Round(2*xdbfunc.DbMath.Round(r.DATA,0),0)==4 && r.TEST_ID==testID);
     ...
     var sqlt
      =new TestSqlTemplate()
        .T("SELECT ").N("n","TEST_ID").T(", ").N("n",c_NameOf__COL_DATA).EOL()
        .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("n").EOL()
        .T("WHERE (ROUND(2.0 * ROUND(").N("n",c_NameOf__COL_DATA).T(", 0), 0) = 4.0) AND (").N("n","TEST_ID").T(" = ").P_ID("__testID_0").T(")");
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => xdbfunc.DbMath.Round(r.DATA,null)==null && r.TEST_ID==testID);
 ...
 //-----------------------------------------------------------------------
 [Test]
 public static void Test___05___ByDBMS____r()
 {
  using(var cn=LocalCnHelper.CreateCn())
  {
   cn.Open();
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => null==xdbfunc.DbMath.Round(r.DATA,null) && r.TEST_ID==testID);
