...
namespace EFCore_LcpiOleDb_Tests.General.Work.DBMS.Firebird.V03_0_0.D3.Query.Operators.SET_001.Multiply.Complete.Single.Single{
////////////////////////////////////////////////////////////////////////////////
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     var recs=db.testTable.Where(r => (r.COL_DATA1*r.COL_DATA2)==31.5F && r.TEST_ID==testID);
  ...
  using(var cn=LocalCnHelper.CreateCn())
  {
   ...
   using(var tr=cn.BeginTransaction())
   {
    ...
    using(var db=new MyContext(tr))
    {
     ...
     const T_RESULT c_result=(T_RESULT)c_value1*(T_RESULT)c_value2;

     System.Int64? testID=Helper__InsertRow(db,c_value1,c_value2);

     var recs=db.testTable.Where(r => (r.COL_DATA1*r.COL_DATA2)==c_result && r.TEST_ID==testID);
     ...
     var sqlt
      =new TestSqlTemplate()
       .T("SELECT ").N("t","TEST_ID").T(", ").N("t",c_NameOf__COL_DATA1).T(", ").N("t",c_NameOf__COL_DATA2).EOL()
       .T("FROM ").N(c_NameOf__TABLE).T(" AS ").N("t").EOL()
       .T("WHERE ((").N("t",c_NameOf__COL_DATA1).T(" * ").N("t",c_NameOf__COL_DATA2).T(") = 1.1579207543382391E+77) AND (").N("t","TEST_ID").T(" = ").P_ID("__testID_0").T(")");
