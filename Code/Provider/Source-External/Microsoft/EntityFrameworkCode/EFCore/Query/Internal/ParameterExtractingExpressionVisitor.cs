...
namespace Lcpi.EXT.Microsoft.EntityFrameworkCore.Query.Internal
{
    ...
    /// </summary>
    /*LCPI: public*/ class ParameterExtractingExpressionVisitor : ExpressionVisitor
    {
        ...
        private bool PreserveConvertNode(Expression expression)
        {
                    ...
                    || unaryExpression.NodeType == ExpressionType.ConvertChecked))
            {
                ...
                {
                    //
                    // IMPORTANT. Saving of operand type for future processing.
                    //

                    return true;
                }

                if (unaryExpression.Type == typeof(Enum))
                {
                    return true;
                }

                // LCPI: if(unaryExpression.Operand.Type.UnwrapNullableType().IsEnum)
                // LCPI: {
                // LCPI:     return true;
                // LCPI: }

                //LCPI: var innerType = unaryExpression.Operand.Type.UnwrapNullableType();

                //LCPI: targetType=unaryExpression.Type.UnwrapNullableType();

                /*LCPI - look to underlying data types*/
                var innerType  =LCPI__Helper__ExtractUnderlyingValueType(unaryExpression.Operand.Type);

                var targetType =LCPI__Helper__ExtractUnderlyingValueType(unaryExpression.Type);

                /*LCPI - saving of conversion between equal types. Will be ignored in future.*/
                if(targetType == innerType)
                {
                    return true;
                }
                ...
                //
                // [2020-12-06] About conversion to decimal:
                //
                // 7 / (decimal)2 == 3.5m
                // 7 / 2 == 3
                //

                //LCPI - [2021-01-20] BUG? We examine the current node only.
                //LCPI: return PreserveConvertNode(unaryExpression.Operand);
        ...
        public static System.Type LCPI__Helper__ExtractUnderlyingValueType(System.Type type)
        {
            ...
            return type;
        }//LCPI__Helper__ExtractUnderlyingValueType
        ...
        //LCPI: private static Expression GenerateConstantExpression(object? value, Type returnType)

        protected virtual Expression LCPI__GenerateConstantExpression(object? value, Type returnType)
        {
        ...
        protected virtual Expression LCPI__GenerateParameterExpression(string  parameterName,
                                                                       object? parameterValue,
                                                                       Type    returnType)
        {
            return Expression.Parameter(returnType, parameterName);
        }
        ...
        private Expression Evaluate(Expression expression, bool generateParameter)
        {
            ...
            if (parameterName?.StartsWith(QueryFilterPrefix, StringComparison.Ordinal) != true)
            {
                ...
                {
                    var constantValue = LCPI__GenerateConstantExpression(parameterValue, expression.Type);

                    Debug.Assert(!Object.ReferenceEquals(constantValue,null));
                    Debug.Assert(!Object.ReferenceEquals(constantValue.Type,null));
                    Debug.Assert(constantValue.Type==expression.Type);
            ...
            var parameter = LCPI__GenerateParameterExpression(parameterName, parameterValue, expression.Type);

            Debug.Assert(!Object.ReferenceEquals(parameter,null));
            Debug.Assert(!Object.ReferenceEquals(parameter.Type,null));
            Debug.Assert(parameter.Type==expression.Type);
        ...
        protected /*LCPI*/virtual object? GetValue(Expression? expression, out string? parameterName)
        {
            parameterName = null;
