...
////////////////////////////////////////////////////////////////////////////////
//class SSCode_Unary_Convert__CastAs__Int64ToDecimal

sealed class SSCode_Unary_Convert__CastAs__Int64ToDecimal
 :FB_Common__Sql_ENode_Unary__Convert.SSCode
{
 private const ErrSourceID
  c_ErrSrcID
   =ErrSourceID.FB_V03_0_0_D3__Sql_ENode__SSCode_Unary_Convert__CastAs__Int64ToDecimal;

 //-----------------------------------------------------------------------
 public static readonly FB_Common__Sql_ENode_Unary__Convert.SSCode
  Instance
   =new SSCode_Unary_Convert__CastAs__Int64ToDecimal();

 //-----------------------------------------------------------------------
 private SSCode_Unary_Convert__CastAs__Int64ToDecimal()
 {
 }//SSCode_Unary_Convert__CastAs__Int64ToDecimal
                                  ...
                                  FB_Common__Sql_ENode_Unary__Convert sqlNode)
 {
  ...
  Debug.Assert(sqlNode.Operand.TypeMapping.DbType==DbType.Int64);
  ...
  {
   //ERROR - required precision isn't supported by the current (3) dialect
 ...
 private static bool Helper__UseArgumentAsIs(SqlUnaryExpression sqlNode)
 {
  ...
  {
   //
   // DECIMAL_NUMERIC( 9-) based on INTEGER
   // DECIMAL_NUMERIC(10+) based on BIGINT+
   //

   Debug.Assert(IscBase_Const.c_SqlPrecision__NUMERIC__Int32==9);

   if(IscBase_Const.c_SqlPrecision__NUMERIC__Int32<sqlNode.TypeMapping.Precision.Value)
   {
    //TARGET based on BIGINT. Can Ignore
   }
   else
   {
    //BIGINT -> NUMERIC_BASED_ON_INTEGER-
